...
function serviceCallback_sub02(svcId, result){
	...
	switch(svcId){
		case "getSurviveItemList":
			var table = $('#gridCensoredDataList').DataTable();
			table.row.add(result.dsItemMgmtList[0]).draw();
...
$(document).on('change','.selAGG_CD',function(e){
	var $element = $(this).parent();
...
$(document).on('click','#gridEventList tbody td',function(e){
	$('#gridEventList').deleteRow(this);
});


$(document).on('click','#gridCensoredDataList tbody td',function(e){
	$('#gridCensoredDataList').deleteRow(this);
});
...
//	생존분석	
	$('#btnCohortSurvive').on('click', function(e){
		
		if( gvSurviveFlagYN === 'N'){
			...
			for(var i=0; i < gvSurviveCensoredCdList.length; i++){
				var dataSet = {};
				
				dataSet.SEARCH_SCHEMA = gvSurviveCensoredCdList[i].COMM_CD_EXT1;
				dataSet.SEARCH_TABLE = gvSurviveCensoredCdList[i].COMM_CD_EXT2;
				dataSet.SEARCH_COLUMN = gvSurviveCensoredCdList[i].VALUE;
				
				if(gvSurviveCensoredCdList[i].COMM_CD_DESC === 'CENSORED_02'){
					dataSet.SEARCH_CH_CENSORING_YN = 'Y';	
				}else{
					dataSet.SEARCH_CH_CENSORING_YN = '';	
				}
				
				
				callServiceSync("getSurviveItemList","common/sys/getItemMgmtList",dataSet,"serviceCallback_sub02");
			...
			$('#gridEventList').initGrid();
			$('#gridCensoredDataList').initGrid();
	...
	$('#btnEventInitGroup').on('click',function(e){
		$('#gridEventList').groupInit();
		
		
	});
	
	$('#btnEventInit').on('click',function(e){
		$('#gridEventList').initGrid();
	});
	
	
	$('#btnCensoredInitGroup').on('click',function(e){
		$('#gridCensoredDataList').groupInit();
		
		
	});
	
	$('#btnCensoredInit').on('click',function(e){
		$('#gridCensoredDataList').initGrid();
	});
	
	
	$('#btnAutoColumns_02').on('click', function(){
		var viewFlag = $("#tabResult_02").html();
...
function getQueryResultList()
{
	...
	//dataSet.SURVIVE_FLAG_YN = gvSurviveFlagYN;	//생존분석조회 여부
	dataSet.SURVIVE_FLAG_YN = 'Y';	//생존분석조회 여부
	
	dataSet.dsItemTableList 	= getSearchTableData();								//스키마, 테이블 목록
	dataSet.dsItemColumnList 	= $('#gridSearch_02').getData();					//조회조건 목록
	dataSet.dsPeriodList 		= [];												//반복관리 목록
	dataSet.dsStudyItem			= $('#gridStudyItem').getGridStudyItem();			//연구항목 목록
	dataSet.dsEventList 		= $('#gridEventList').getGridSurvivalAnalysis();		//생존분석 - 사건정의
	dataSet.dsCensoredDataList 	= $('#gridCensoredDataList').getGridSurvivalAnalysis();	//생존분석 - 중도절단
...
//	사건정의	
	if(!$('#gridEventList').validDate()){
		showAlert(null,COM_0032,null);
		return;
	}
	
//	생존분석	
	if(!$('#gridCensoredDataList').validDate()){
		showAlert(null,COM_0032,null);
		return;
	}
	
	callService("getStudyItemTargetList"
				,"research/cohort/searchStudyItemTargetList"
				,dataSet
...
//	ITEM_CONT	
	...
	dataSet.CC_CONT_CD 	= '0';							//(사례대조) 대조군 배수
