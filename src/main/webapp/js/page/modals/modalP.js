...
function serviceCallbackP(svcId, result){
	...
	switch(svcId){			
		...
		case "getDataP":
			...
			/*if(jObj.INSTCD_YN == "Y"){
				//사업장명 추가
				for(var i=0; i<result.dsCodeListP.length; i++){
					result.dsCodeListP[i]['INSTCDNM'] = gvArrInstcdNm[result.dsCodeListP[i]['INSTCD']];
				}
			}*/
		...
		case "getParentDataP":
			...
			/*if(jObj.INSTCD_YN == "Y"){
				//사업장명 추가
				for(var i=0; i<result.dsDiseaseParentCodeListP.length; i++){
					result.dsDiseaseParentCodeListP[i]['INSTCDNM'] = gvArrInstcdNm[result.dsDiseaseParentCodeListP[i]['INSTCD']];
				}
			}*/
...
function getDataP(){	
	...
	callServiceSpinnerClose("getDataP" ,"common/modal/getCodeListP" ,dataSet ,"serviceCallbackP");
}
...
//	---------------------------------------------------
	...
	/*if(jObj.INSTCD_YN == "Y"){
		if(gridDatafieldsPVal[0]['name'] != 'INSTCD'){
			gridDatafieldsPVal.unshift({"name" : "INSTCDNM"});
			gridDatafieldsPVal.unshift({"name" : "INSTCD"});
			gridColumnsPVal.unshift({"text" : "병원명","datafield" : "INSTCDNM","width":"100"});
			gridColumnsPVal.unshift({"text" : "병원구분코드","datafield" : "INSTCD","width":"100"});
		}
	}*/
...
*/		var columns = [
				    	{ text: '처방코드', datafield: 'CODE', width: "20%"},
...
//	동기방식	
	callService("getDiseaseCodeTreeP", "/common/modal/getDiseaseCodeTreeP", dataSet, "serviceCallbackP");
...
*/					if(dsPerAuth.CODE === dsPerinx.CODE){
	...
	$(document).on('click', '#cancelItemP', function(){
		var rows = $("#searchSelectPGrid").jqxGrid('selectedrowindexes');
        var selectedRecords = new Array();
        
        for(var i = (rows.length-1); i >= 0; i--) {
            var row = $("#searchSelectPGrid").jqxGrid('getrowdata', rows[i]);
            $("#searchSelectPGrid").jqxGrid('deleterow', row.uid);
        }
	});
...
*/					retObj += '|' + dsValue.CODE;
	...
	$('#newSaveP').on('ifChecked', function(event){
		$('#lovNmP').prop('disabled', false);
		$('#lovNmP').val('');
	});
	
	$('#saveP').on('ifChecked', function(event){
		$('#lovNmP').prop('disabled', true);
		$('#lovNmP').val($('#saveNameP').val());
	});
}
...
var gvPopPSel = {
		'A' : [
		       	["ORD_KIND", "ORD_KIND", "ORD_KIND_NM", "CC_ORDR_CD", "ORD_KIND", "''", "!="],
		       	["ORD_SCLS", "ORD_SCLS", "ORD_SCLS_NM", "CC_ORDR_CD", "ORD_SCLS", "''", "!="]
		       ],
		'B' : [
		       	["LB_SL_CODE", "LB_SL_CODE", "LB_SL_NM", "CC_ORDR_CD", "LB_SL_CODE", "''", "!="]
		       ],
		'C' : [
		       	["RAD_PACSPECIALITY", "ORD_SCLS", "ORD_SCLS_NM", "CC_ORDR_CD", "left(ORD_SCLS,1)", "'R'", "="]
		       ],
		'D' : [
		       	["SP_SL_CODE", "ORD_SCLS", "ORD_SCLS_NM", "CC_ORDR_CD", "left(ORD_SCLS,1)", "'S'", "="]
		       ],
		'E' : [
		       	["PATH_SL_CODE", "ORD_SCLS", "ORD_SCLS_NM", "CC_ORDR_CD", "left(ORD_SCLS,2)", "('P1','P2','P3','P4','P5','P6','P7','P8','P9')", "in"]
		       ],
		'F' : [
		       	["MB_THRU_CHAN"],
		       	["MB_CLSS_COED", "MB_CLSS_COED", "MB_CLSS_NM", "CC_ORDR_CD", "MB_CLSS_COED", "''", "!="]
		       ],
		'G' : [
		       	["OP_SL_CODE", "ORD_SCLS", "ORD_SCLS_NM", "CC_ORDR_CD", "left(ORD_SCLS,2)", "('H2','H3','H4','H5','H6','H7','H8','H9')", "in"]
		       ]
