	...
	switch(svcId){
		case "getCommonCodeList":
			...
			if(isNull(objVal2HTMR)){					//넘어온 값이 없을때
				...
				var objVal2HTMRSplit = objVal2HTMR.split(gvSplitChar);
				...
				$.each(items, function () {
					if(this.value == objVal2HTMRSplit[0]) {
						...
						$("#codeTree2HTMR").jqxTree('ensureVisible', this.element);
						return false;
					}
				});
				/*var objVal2HTMRSplit = objVal2HTMR.split(gvSplitChar);		//1H, 2H분리
		...
		case "getUpperCommonCodeList":
			...
			if(searchWrd){
				queryCommonCodeList = replaceAll(queryCommonCodeList, '#{CODE_NM}', "'"+ searchWrd +"'");
			}else{
				queryCommonCodeList = replaceAll(queryCommonCodeList, '#{CODE_NM}', "''");
			}
...
function setGrid2HTMRSelect()
{
	...
	/*if(jObj.INSTCD_YN == "Y"){
		...
		var columns = [
				        ...
				        { text: '항목타입', datafield: 'type_name', width: "10%"},
				        { text: '항목타입코드', datafield: 'UMR_ITYPE', width: "10%"},
...
*/		var datafields = [
		...
		var columns = [
			        ...
			        { text: '항목명', datafield: 'CODE_NM', width: "25%"},
...
function initEvent2HTMR(){	
	...
	$(document).on('select', '#codeTree2HTMR', function (event) {
		...
		if(level == level1){
			...
			//사업장코드가 있을때
			if(jObj.INSTCD_YN == "Y"){
				replaceQuery = replaceAll(replaceQuery,"'%'||#{CODE_NM}||'%'", "'%'||''||'%' AND A.INSTCD IN (" + strINSTCD2 + ")");
				replaceQuery = replaceAll(replaceQuery,"'%'|| #{CODE_NM} ||'%'", "'%'||''||'%' AND A.INSTCD IN (" + strINSTCD2 + ")");
				replaceQuery = replaceAll(replaceQuery,"'%' ||#{CODE_NM}|| '%'", "'%'||''||'%' AND A.INSTCD IN (" + strINSTCD2 + ")");
				replaceQuery = replaceAll(replaceQuery,"'%' || #{CODE_NM} || '%'", "'%'||''||'%' AND A.INSTCD IN (" + strINSTCD2 + ")");
