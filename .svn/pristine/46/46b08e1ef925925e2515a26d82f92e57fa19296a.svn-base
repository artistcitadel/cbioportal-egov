...
function serviceCallback(svcId, result){
	switch(svcId){
		...
		case "insertItemCate":
			showAlert('연구분류관리',COM_0001,function(result){
		...
		case "updateItemCate":
			showAlert('연구분류관리',COM_0002,function(result){
				$('#btnClose').trigger('click');
				getItemCateList();
			});
		...
		case "deleteItemCate":
			showAlert('연구분류관리',COM_0003,function(result){
				$('#btnClose').trigger('click');
				getItemCateList();
			});
		...
		columns : [
			{
				...
				text : '표시순서', 	datafield : 'ORDER', 	width : '30%' ,	cellsalign:'center',
...
//	테이블선택
	...
	$('#btnDel').on('click',function(e){
		...
		showConfirm("연구분류관리", strMsg, function(result){
			if(!result) {
                return;
            }
	...
	//	대분류seq설정
		...
		$('#selSchemaList').attr('disabled',false);
		$('#selTableList').attr('disabled',false);
...
//		대분류seq설정
	...
	$('#btnDetlDel').on('click',function(e){
		showConfirm("연구분류관리", COM_0005, function(result){
			if(!result) {
                return;
            }
	...
	$('#btnSave').on('click',function(e){
		showConfirm("연구분류관리", COM_0004, function(result){
			if(!result) {
                return;
            }
	...
	$('#btnDetlSave').on('click',function(e)
	{
		...
		if(isNull($('#txtCATE_NM_DETL').val())){
			showAlert('연구분류관리','분류명은 ' + COM_0010,null);
			return;
